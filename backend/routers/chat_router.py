...
async def _get_user_llm_model(db, user_id: int) -> Optional[str]:
    """获取用户选择的 LLM 模型（用于覆盖默认模型）"""
    cursor = await db.execute(
        "SELECT llm_model FROM user_settings WHERE user_id = ?", (user_id,)
    )
    row = await cursor.fetchone()
    if row and row[0]:
        return row[0]
...
async def _get_user_asr_model(db, user_id: int) -> Optional[str]:
    """获取用户选择的 ASR 模型"""
    cursor = await db.execute(
        "SELECT asr_model FROM user_settings WHERE user_id = ?", (user_id,)
    )
    row = await cursor.fetchone()
    if row and row[0]:
        return row[0]
...
async def text_chat(data: TextChatRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 读取用户选择的模型
        user_model = await _get_user_llm_model(db, current_user["id"])
        ...
        async def generate():
            ...
            try:
                async for chunk in llm_service.chat_stream(history, model=user_model):
                    full_response += chunk
...
async def check_wake_word(
    ...
    try:
        ...
        asr_db = await get_db()
        try:
            user_asr = await _get_user_asr_model(asr_db, current_user["id"])
        finally:
            await asr_db.close()
        text = await asr_service.recognize(audio_data, is_wav=is_wav, model=user_asr)
...
async def voice_chat(
    ...
    try:
        ...
        # 2. ASR 语音识别（使用用户选择的 ASR 模型）
        user_asr = await _get_user_asr_model(db, current_user["id"])
        asr_text = await asr_service.recognize(audio_data, is_wav=is_wav, model=user_asr)
        ...
        history = await _get_session_messages(db, session_id)
        user_model = await _get_user_llm_model(db, current_user["id"])
        reply_text = await llm_service.chat(history, model=user_model)
        ...
        while True:
            ...
            if msg_type == "text":
                ...
                try:
                    ...
                    history = await _get_session_messages(db, session_id)
                    user_model = await _get_user_llm_model(db, user["id"])
            ...
            # 语音消息
                ...
                try:
                    ...
                    # ASR（用户选择的模型）
                    user_asr = await _get_user_asr_model(db, user["id"])
                    asr_text = await asr_service.recognize(audio_data, is_wav=True, model=user_asr)
...
async def deep_research(data: DeepResearchRequest, current_user: dict = Depends(get_current_user)):
    ...
    基于 DashScope qwen-deep-research 模型（原生 SDK，非 OpenAI 兼容接口）
    """
    db = await get_db()
    try:
        session_id = await _get_or_create_session(db, current_user["id"], data.session_id)

        # ⚠️ 必须先获取 history，再保存当前问题
        # 否则 history 会包含当前问题，导致传入 DashScope 时当前问题被重复出现
        history = await _get_session_messages(db, session_id)

        # 保存用户消息（在获取 history 之后）
        ...
        msg_count = (await cursor.fetchone())[0]
        if msg_count <= 1:
            title = await llm_service.generate_title(data.question)
            await db.execute("UPDATE sessions SET title = ? WHERE id = ?", (title, session_id))
            await db.commit()
            logger.info(f"[DeepResearch] 已生成会话标题（generate_title 使用普通LLM，非DR调用）: '{title}'")

        logger.info(
            f"[DeepResearch] 开始深度研究 | session={session_id} | "
            f"question='{data.question[:60]}' | history_len={len(history)}"
        )
        ...
        async def generate():
            ...
            try:
                ...
                logger.info(
                    f"[DeepResearch] 研究完成 | session={session_id} | "
                ...
                logger.exception(f"[DeepResearch] 流式生成出错: {e}")
                yield f"data: {json.dumps({'type': 'error', 'message': str(e)})}\n\n"
