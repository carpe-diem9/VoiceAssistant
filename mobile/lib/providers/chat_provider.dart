...
class ChatProvider extends ChangeNotifier {
  ...
  // Deep Research 模式开关（启用后文本消息会走 tongyi-deepresearch）
  ...
  bool get deepResearchMode => _deepResearchMode;
  ...
  /// 切换 Deep Research 模式开关
  Future<void> toggleDeepResearch() async {
    _deepResearchMode = !_deepResearchMode;
    if (_deepResearchMode) {
      // Deep Research 模式下默认关闭 TTS，结果长不适合朗读
      _ttsEnabled = false;
      final prefs = await SharedPreferences.getInstance();
  ...
  /// 设置 Deep Research 模式
  Future<void> setDeepResearchMode(bool value) async {
    if (_deepResearchMode == value) return;
    _deepResearchMode = value;
    if (_deepResearchMode) {
      _ttsEnabled = false;
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('tts_enabled', false);
  ...
  Future<void> sendTextMessage(String text) async {
    ...
    // 添加用户消息
    final userDisplay = _deepResearchMode ? '🔬 [Deep Research] $text' : text;
    _messages.add(Message(role: 'user', content: userDisplay));
    ...
    if (_deepResearchMode) {
      try {
        ...
        String thinking = '';
        bool enteredFinal = false;

        String render() {
          final sb = StringBuffer();
          if (steps.isNotEmpty) sb.writeln(steps.join('\n'));
        ...
        await for (final data in _api.deepResearchStream(text, sessionId: _currentSessionId)) {
          ...
          if (type == 'session_id') {
            ...
            steps.add('✨ 步骤 $step: $content');
            assistantMsg.content = render();
            notifyListeners();
          } else if (type == 'thinking') {
            thinking += (data['content'] ?? '') as String;
            assistantMsg.content = render();
            notifyListeners();
          } else if (type == 'final_chunk') {
            if (!enteredFinal) {
              enteredFinal = true;
              final header = render();
              assistantMsg.content = header.isEmpty ? '---\n' : '$header\n---\n';
            }
            assistantMsg.content += (data['content'] ?? '') as String;
            notifyListeners();
          } else if (type == 'final') {
            // 兼容后端一次性返回 final 的场景
            final full = (data['content'] ?? '') as String;
            if (full.isNotEmpty && !assistantMsg.content.contains(full)) {
              final header = render();
