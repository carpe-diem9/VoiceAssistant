...
class ApiService {
  ...
  Future<void> updateASRModel(String model) async {
    await _put('${AppConfig.apiSettings}/models', {'asr_model': model});
  }

  Future<void> updateTTSModel(String model) async {
    await _put('${AppConfig.apiSettings}/models', {'tts_model': model});
  }

  // ===== Accessibility =====
  /// 无障碍朗读：原文清洗或 LLM 总结，可选同步返回 TTS 音频
  /// style: 'read' 或 'summary'
  /// 返回: {text, audio_base64?, audio_format}
  Future<Map<String, dynamic>> readText(
    String text, {
    String style = 'summary',
    bool enableTts = true,
  }) async {
    return await _post('${AppConfig.apiAccessibility}/read-text', {
      'text': text,
      'style': style,
      'enable_tts': enableTts,
    });
